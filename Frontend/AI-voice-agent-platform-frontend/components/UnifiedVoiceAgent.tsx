...
import { AgentProcessingOverlay } from "./AgentProcessingOverlay";
...
interface UnifiedVoiceAgentProps {
  ...
  className?: string;
  assistantType?: "landing" | "onboarding" | "improvement"; // Added new "improvement" type
...
export function UnifiedVoiceAgent({ 
  ...
  const connectToVoiceAssistant = useCallback(async () => {
    // Don't allow connecting while processing feedback
  ...
  }, [assistantName, assistantType, isProcessingFeedback]);
  ...
  // Force disconnect when feedback processing starts
  useEffect(() => {
    if (isProcessingFeedback && connectionDetails) {
      handleDisconnect();
    }
  }, [isProcessingFeedback, connectionDetails, handleDisconnect]);
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="relative">
              ...
              {connectionDetails === undefined && (
                ...
                >
                  <button 
                    ...
                    className={`h-24 w-24 rounded-full ${isProcessingFeedback ? 'bg-purple-600/50 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 cursor-pointer'} transition-colors flex items-center justify-center shadow-lg`}
                  ...
                  <p className="text-white/80 text-base font-medium">
                    {isProcessingFeedback 
                      ? "WAITING FOR ALICE TO FINISH UPDATES" 
        ...
        <AnimatePresence>
          {isProcessingFeedback && (
            <AgentProcessingOverlay isProcessing={isProcessingFeedback} />
          )}
        </AnimatePresence>
      </div>
    </div>
